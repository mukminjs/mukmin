  ...
  await loop(Object.keys(datastores), async datastoreKey => {
    ...
    await loop(files, async file => {
      ...
      if (fileNameSplit[1] === "model") {
        ...
        // await global[modelName].sync({ force: true });
      }
    });
      ...
      var modelSchema = require("../../app/models/" +
        ...
        await loop(await modelSchema.associations, async association => {
          ...
          if (association.type === "belongsTo") {
            console.log(chalk.bgGreen.black("Model name : "+modelName+' belongsTo '+association.model));      
            modelDB[modelName].belongsTo(
              modelDB[association.model],
              association.options
            );
          } else if (association.type === "hasMany") {
            console.log(chalk.bgGreen.black("Model name : "+modelName+' hasMany '+association.model));      
            modelDB[modelName].hasMany(
              modelDB[association.model],
              association.options
            );
          }else if( association.type === 'belongsToMany'){
            console.log(chalk.bgGreen.black("Model name : "+modelName+' belongsToMany '+association.model));      
